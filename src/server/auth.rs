...
pub async fn register_user(
    ...
    response.insert_header(
        ...
        axum::http::HeaderValue::from_str(&session::make_session_cookie(&session_id))
...
pub async fn logout_user() -> Result<(), ServerFnError> {
    ...
    if let Some(sid) = session_id {
        let _ = session::delete_session(&pool, &sid).await;
    }

    let response = expect_context::<ResponseOptions>();
    response.insert_header(
        axum::http::header::SET_COOKIE,
        axum::http::HeaderValue::from_str(&session::clear_session_cookie())
            .map_err(|e| ServerFnError::new(format!("Cookie error: {e}")))?,
    );
...
pub async fn get_current_user() -> Result<Option<CurrentUser>, ServerFnError> {
    ...
    let headers: HeaderMap = leptos_axum::extract().await
        .map_err(|e| ServerFnError::new(e.to_string()))?;
    let session_id = crate::server::session::session_id_from_headers(&headers);

    let session = crate::server::session::get_session(&pool, session_id.as_deref()).await;
...
pub async fn set_theme(theme: String) -> Result<(), ServerFnError> {
    ...
    let headers: HeaderMap = leptos_axum::extract()
        ...
        .map_err(|e| ServerFnError::new(e.to_string()))?;
    let session_id = crate::server::session::session_id_from_headers(&headers);
    let session = crate::server::session::get_session(&pool, session_id.as_deref())
        .await
